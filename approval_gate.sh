...
DESTRUCTIVE='(\brm\b.*-[rf]|systemctl (stop|disable)|kubectl delete|reboot|shutdown|drop\s+table|dd\s+if=|journalctl.*--vacuum|resize-pvc)'
...
if printf '%s' "$cmd" | grep -Eiq "$DESTRUCTIVE"; then
  TOKEN="${1:-${HUMAN_APPROVAL_TOKEN:-}}"
  if [[ "$TOKEN" == "I-APPROVE" ]]; then
